...
(
	...
	CONSTRAINT FK_EXTRACT_BATCH_FLEX_VAR FOREIGN KEY (BATCH_ID) REFERENCES EXTRACT_BATCH (BATCH_ID) ON DELETE CASCADE
);

COMMENT ON TABLE EXTRACT_BATCH_FLEX_VAR
	IS '(Optional) variables addition to EXTRACT_BATCH.';

COMMENT ON COLUMN EXTRACT_BATCH_FLEX_VAR.VAR_NAME
	IS 'The variable name';
COMMENT ON COLUMN EXTRACT_BATCH_FLEX_VAR.VAR$DATE
	IS 'Use this column if the variable is compatible with DATE type.';
COMMENT ON COLUMN EXTRACT_BATCH_FLEX_VAR.VAR$NUM
	IS 'Use this column if the variable is compatible with NUMBER type.';
COMMENT ON COLUMN EXTRACT_BATCH_FLEX_VAR.VAR$STR
